...
<script>
	$(document).ready(function(){
		$("#insert").click(function(){
			location.href = "/admin/category/write.do";
		});
...
<div id="Wrapper">
    ...
    <div id="Content"><!--Content-->
        ...
        <div class="sch">
        	분류 <form:select path="category.category_no" cssClass="txt01">
						<form:option value="">전체</form:option>
						<form:options items="${select}" itemLabel="title" itemValue="value" />
					</form:select> <span id="category_search" class="glay_big">검색</span>
        </div>
       	</form>
		<div class="con">
			<table class="info">
				...
				<c:choose>
					<c:when test="${fn:length(list) >0 }">
						<c:forEach items="${list }" var="data" varStatus="status">
							<form action="/admin/category/write.do" method="get">
							<input type="hidden" name="category_no" value="${data.category_no}" />
							<input type="hidden" name="org_category_no" value="${category.category_no}" />
							<tr>
								...
								<td>
									<c:choose>
										<c:when test="${data.parent_category_no > 0 }">
											${data.parent_category_name}(${data.parent_category_name_ko})
										</c:when>
