...
<script>
	$(document).ready(function(){
		...
		$("#insert").click(function(){
			location.href = "/admin/community/notice_write.do?page=${list.page}";
		});
	...
	var params = "&search_type=${notice.search_type}&search_string=${notice.search_string}";
...
<div id="Wrapper">
    ...
    <div id="Content"><!--Content-->
    	<div class="tit">공지관리</div>
    	
    	<form id="frm_search" method="post">
        <div class="sch">
			<form:select path="notice.search_type" cssClass="txt01" cssStyle="width:80px;" >
            	<form:option value="title">제목</form:option>
            	<form:option value="content">내용</form:option>
            </form:select> 
            <form:input path="notice.search_string" cssClass="txt01" cssStyle="width:120px;" />
				...
				<c:choose>
					<c:when test="${fn:length(list) >0 }">
						<c:forEach items="${list }" var="data" varStatus="status">
							<tr>
								<td>${data.no}</td>
								<td><a href="/admin/community/notice_write.do?page=${list.page}&no=${data.no}">${data.title}</a></td>
								<td>${data.user_name}</td>
								<td>
									<c:if test="${data.top == 'Y'}"><span class="red">사용</span></c:if>
									<c:if test="${data.top == 'N'}">미사용</c:if>
								</td>
								<td>${data.hit}</td>
								<td>${data.reg_date}</td>
