...
<script>
	$(document).ready(function(){
		$("#insert").click(function(){
			location.href = "/admin/coupon/write.do";
		});
	});
</script>
</head>
				...
				<c:choose>
					<c:when test="${fn:length(list) >0 }">
						<c:forEach items="${list }" var="data" varStatus="status">
							<form action="/admin/coupon/write.do" method="get">
							<input type="hidden" name="category_no" value="${data.idx}" />
							<tr>
								<td>${data.idx}</td>
								<td>${data.coupon_type}(${data.coupon_value})</td>
								<td>${data.desc}</td>
