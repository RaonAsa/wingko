...
<script>
	$(document).ready(function(){
		...
		$("#insert").click(function(){
			location.href = "/admin/ad/write.do?ad_no=${adData.ad_no}";
		});
		
/* 		$("#ad_search").click(function() {
			$("#frm_search").submit();
		}); */
		$("#ad_no").change(function() {
			$("#frm_search").submit();
		});
		...
		$("#chk_use").click(function() {
			location.href = "/admin/ad/list.do?ad_no=${adData.ad_no}&chk_use="+$(this).is(":checked");
		});
	});
...
<div id="Wrapper">
    ...
    <div id="Content"><!--Content-->
    	<div class="tit">광고관리</div>
    	<form id="frm_search" method="get">
        <div class="sch">
        	광고위치 <form:select path="adData.ad_no" cssClass="txt01">
						<form:options items="${select}" itemLabel="title" itemValue="value" />
					</form:select><!--  <span id="ad_search" class="glay_big">검색</span> -->
			사용중인 광고만 보기 <input type="checkbox" name="chk_use" id="chk_use" />
        </div>
       	</form>
		<div class="con">
			<table class="info">
				...
				<c:choose>
					<c:when test="${fn:length(list) >0 }">
						<c:forEach items="${list }" var="data" varStatus="status">
							<form action="/admin/ad/write.do" method="get">
							<input type="hidden" name="no" value="${data.no}" />
							<input type="hidden" name="ad_no" value="${data.ad_no}" />
