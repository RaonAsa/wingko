...
<script>
	$(document).ready(function(){
		$("#insert").click(function(){
			location.href = "/admin/store/coupon_write.do?store_no=${storeCoupon.store_no}&page=${page}";
		});
				...
				<c:choose>
					<c:when test="${fn:length(list) >0 }">
						<c:forEach items="${list }" var="data" varStatus="status">
							<form action="/admin/store/coupon_write.do" method="get">
							<input type="hidden" name="page" value="${page}" />
							<input type="hidden" name="coupon_no" value="${data.coupon_no}" />
							<tr>
								<td>${data.coupon_no}</td>
